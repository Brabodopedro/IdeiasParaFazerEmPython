    ...
    def Processing_incoming_messages(self):
        ...
        if message:
            if 'body' in message and 'from' in message:
                text = message['body'].strip()  # Captura o texto da mensagem enviada pelo usuário
                if not message.get('fromMe', False):  # Ignora mensagens enviadas pelo bot
                    ...
                    if state == 'INITIAL':
                        ...
                        if text == '1':
                            ...
                            self.send_message(self.chatID, "Opção inválida. Por favor, responda com o número correspondente à sua escolha.")
                        ...
                        if text.lower() in ['sim', 's']:
                            self.send_message(self.chatID, "Obrigado por sua compra! Entraremos em contato para finalizar os detalhes.")
                            self.states[self.chatID]['state'] = 'FINISHED'
                            return 'Purchase confirmed'
                        elif text.lower() == 'n':
                            self.handle_buy_device()
                            return 'Restarted model selection'
                        elif text.lower() == 'm':
                            self.send_options()
                            self.states[self.chatID]['state'] = 'ASKED_OPTION'
                            return 'Returned to main menu'
                        elif text.lower() == 's':
                            self.send_message(self.chatID, "Obrigado pelo contato!")
                            self.states[self.chatID]['state'] = 'FINISHED'
                            return 'Exited conversation'
                        else:
                            self.send_message(self.chatID, "Opção inválida. Por favor, responda com 'Sim', 'N', 'M' ou 'S'.")
                            return 'Invalid confirmation option'
                    elif state == 'ASKED_MODEL_NAME':
                        model_name = text
